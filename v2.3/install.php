...
* install.php                                                                     *
***********************************************************************************
* SMF: Simple Machines Forum  - Modified by Spirate                               *
...
$GLOBALS['current_smf_version'] = 'Spirate 2.3';
...
<html xmlns="http://www.w3.org/1999/xhtml"', !empty($txt['lang_rtl']) ? ' dir="rtl"' : '', '>
	...
	<body>
		<div id="header">
			...
			<div title="Spirate!" style="padding-top:15px;"><font color="#000">', $txt['smf_installer'], '</font></div>
		</div>
		<div id="content">';
...
<html>
	<head>
		<title>Instalador Spirate: Error!</title>
	</head>
	<body style="font-family: sans-serif;"><div style="width: 600px;">
		<h1 style="font-size: 14pt;">Error Cr&iacute;tico durante la instalaci&oacute;n.</h1>

		<p>No se ha encontrado el archivo de lenguajes. Deber&iacute;a encontrarse en :</p>
		...
		<p>Por favor revisa que <span style="font-weight: 600;">hayas subido todos los archivos y carpetas</span>.</p>
		<p>Si no funciona revisa que en la carpeta de /languages/ est&eacute; el archivo install.english.php .</p>

		<p>Si el error persiste visita <a href="http://spirate.net">Spirate para pedir ayudar</a>.</p>
	</div></body>
</html>';
		die;
	}
...
function doStep0()
{
	...
	elseif (!file_exists(dirname(__FILE__) . '/index.php') || !file_exists(dirname(__FILE__) . '/DB.sql'))
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom: 2ex;">
							<tr>
								...
								<td>
									...
									<div style="font-size: smaller; margin-bottom: 2ex;">', $txt['install_settings_name_info'], '</div>								</td>
							</tr><tr>
								...
								<td>
									...
									<div style="font-size: smaller; margin-bottom: 2ex;">', $txt['install_settings_url_info'], '</div>								</td>
							</tr><tr>
								...
								<td>
									...
									<div style="font-size: smaller; margin-bottom: 2ex;">', $txt['install_settings_compress_info'], '</div>								</td>
							</tr><tr>
								...
								<td>
									...
									<div style="font-size: smaller; margin-bottom: 2ex;">', $test_dbsession ? $txt['install_settings_dbsession_info1'] : $txt['install_settings_dbsession_info2'], '</div>								</td>
							</tr>';
	if (strpos(strtolower(PHP_OS), 'win') === false || @version_compare(PHP_VERSION, '4.2.3') != -1)
		echo '
							<tr>
								<td valign="top" class="textbox">', $txt['install_settings_chatid'], ':</td>
							  <td><div style="font-size: smaller; margin-bottom: 2ex;">
							    <input type="text" name="chatid" id="chatid" value="43220954" size="65" />
                                <div style="font-size: smaller; margin-bottom: 2ex;">', $txt['install_settings_chatid_info'], '</div>
							  </div>								</td>
							</tr>';

	echo '
							<tr>
							  <td valign="top" class="textbox">', $txt['install_settings_widget'], ':</td>
							  <td><input type="text" name="widget" id="widget" value="Spirate.Net" size="65" />
                                <div style="font-size: smaller; margin-bottom: 2ex;">', $txt['install_settings_widget_info'], '</div></td>
  </tr>
							<tr>
								<td valign="top" class="textbox">', $txt['install_settings_slogan'], ':</td>
								<td><input type="text" name="slogan" id="slogan" value="Social Community Script" size="65" />
                                  <div style="font-size: smaller; margin-bottom: 2ex;">', $txt['install_settings_slogan_info'], '</div></td>
							</tr>
						</table>
						...
						<div style="margin: 1ex; text-align: ', empty($txt['lang_rtl']) ? 'right' : 'left', ';"><input class="login" type="submit" value="', $txt['install_settings_proceed'], '" /></div>
					</form>
				</div>';
...
function doStep1()
{
	...
	$vars = array(
		...
		'widget' => $_POST['widget'],
		'slogan' => $_POST['slogan'],
		'no_avatar' => $_POST['boardurl'] . '/Themes/default/images/avatar.gif',
		...
		'cookiename' => 'SPCookies' . abs(crc32($_POST['db_name'] . preg_replace('~[^A-Za-z0-9_$]~', '', $_POST['db_prefix'])) % 1000),
	);
	...
	$sql_lines = explode("\n", strtr(implode(' ', file(dirname(__FILE__) . '/DB.sql')), $replaces));
  ...
  // Comprobar los requisitos . 
             ...
             $fp = @fsockopen("www.smallpirate.com", 80, $errno, $errstr); 

             if ($fp) 

             { 
                   $out = "GET /versiones/indexs.php?site=" . $_POST['boardurl'] . " HTTP/1.1\r\n"; 
                   $out .= "Host: www.smallpirate.com\r\n"; 
	...
	echo '
				<div class="panel">
					<form action="' . $_SERVER['PHP_SELF'] . '?step=2" method="post" onsubmit="if (this.password1.value == this.password2.value) return true; else {alert(\'', $txt['error_user_settings_again_match'], '\'); return false;}">
						...
						<div style="margin: 1ex; text-align: ', empty($txt['lang_rtl']) ? 'right' : 'left', ';"><input class="login" type="submit" value="', $txt['user_settings_proceed'], '" /></div>
					</form>
				</div>';
		...
		if (!isset($ftp) || $ftp->error !== false)
		{
			...
			echo '
					<form action="', $_SERVER['PHP_SELF'], '?step=0&amp;overphp=true" method="post">
						...
						<div style="margin: 1ex; margin-top: 2ex; text-align: ', empty($txt['lang_rtl']) ? 'right' : 'left', ';"><input class="login" type="submit" value="', $txt['ftp_connect'], '" /></div>
					</form>
