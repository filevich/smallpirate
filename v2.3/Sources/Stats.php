...
function DisplayStats()
{
	...
	if (isset($_REQUEST['xml']))
	{
        ...
        if(loadLanguage('Stats') == false)
            loadLanguage('Stats','spanish');

	loadTemplate('Stats');

	isAllowedTo('view_stats');

	$context['page_title'] = $txt['smf_stats_1'] . '';
...
LIMIT 0 , 10", __FILE__, __LINE__);
	...
	SELECT*
	FROM ({$db_prefix}messages AS m, {$db_prefix}comentarios AS c)
	...
	while ($row_topic_views = mysql_fetch_assoc($topic_view_result))
	{
		...
		$context['top_topics_views'][] = array(
			...
			'board' => array(
				...
				'href' => $scripturl . '?id=' . $row_topic_views['ID_BOARD'] . '',
				'link' => '<a href="'. $scripturl .'?id=' . $row_topic_views['ID_BOARD'] . '">' . $row_topic_views['name'] . '</a>'
			),
			'subject' => $row_topic_views['subject'],
			'num_views' => $row_topic_views['numViews'],
			'href' => $scripturl . '?topic=' . $row_topic_views['ID_TOPIC'] . '',
			'link' => '<a href="'. $scripturl .'?topic=' . $row_topic_views['ID_TOPIC'] . '">' . $row_topic_views['subject'] . '</a>'
		);
	...
	$months_result = db_query("
		SELECT
			YEAR(date) AS stats_year, MONTH(date) AS stats_month, SUM(hits) AS hits, SUM(registers) AS registers, SUM(topics) AS topics, SUM(posts) AS posts, MAX(mostOn) AS mostOn, COUNT(*) AS numDays, SUM(referrals) AS referrals
		FROM {$db_prefix}log_activity
	...
	while ($row_months = mysql_fetch_assoc($months_result))
	{
		...
		$context['monthly'][$ID_MONTH] = array(
			...
			'href' => $scripturl . '?action=TOPs' . ($expanded ? 'contraer' : 'expandir') . '/' . $ID_MONTH . '#' . $ID_MONTH,
			'link' => '<a href="'. $scripturl .'?action=TOPs' . ($expanded ? 'contraer' : 'expandir') . '/' . $ID_MONTH . '#' . $ID_MONTH . '">' . $txt['months'][$row_months['stats_month']] . ' ' . $row_months['stats_year'] . '</a>',
			...
			'referrals' => $row_months['referrals'],
...
function getDailyStats($condition)
{
	...
	$days_result = db_query("
		SELECT YEAR(date) AS stats_year, MONTH(date) AS stats_month, DAYOFMONTH(date) AS stats_day, topics, posts, registers, mostOn, referrals, hits
		FROM {$db_prefix}log_activity
		WHERE $condition
...
function SMStats()
{
	...
	else
	{
		...
		$fp = @fsockopen("". $scripturl ."", 80, $errno, $errstr);
		if ($fp)
		{
