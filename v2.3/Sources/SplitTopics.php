...

{
	...
	db_query("
		...
		WHERE ID_MEMBER = $split2_firstMem
...

{
	...
	$deleted_topics = array_diff($topics, array($ID_TOPIC));
		...
		$mmbrsndpdt = db_query("
			...
			WHERE ID_TOPIC IN (" . implode(', ', $deleted_topics) . ")GROUP BY ID_MEMBER_STARTED", __FILE__, __LINE__);



			while ($rowMembers = mysql_fetch_assoc($mmbrsndpdt))

				updateMemberData($rowMembers['ID_MEMBER_STARTED'], array('topics' => 'topics - ' . $rowMembers['topics']));

		mysql_free_result($mmbrsndpdt);
