...
<html>
	...
	<body>
		<h3>No disponible Temporalmente</h3>
		Debido a la tensi&oacute;n alta en el servidor del foro no est&aacute; disponible temporalmente. Por favor, int&eacute;ntelo de nuevo m&aacute;s tarde.
	</body>
</html>';
...
Lo sentimos. SP no pudo conectarse con la base de datos. Esto puede ser causa de una mala configuraci&oacute;n en el sistema o el servidor est&aacute; muy ocupado. Vuelva a intentarlo m&aacute;s tarde.	</body>
</html>';
...
function JSMembers()
{
	...
	if (isset($_REQUEST['search']))
	{
		...
		$base_url = $scripturl . '?action=findmember;search=' . urlencode($context['last_search']) . (empty($_REQUEST['u']) ? '' : ';user=' . $_REQUEST['u']) . ';sesc=' . $context['session_id'];
		$context['links'] = array(
			...
			'up' => $scripturl . '?action=pm;sa=send' . (empty($_REQUEST['u']) ? '' : ';user=' . $_REQUEST['u']),
		);
		...
		$context['links']['up'] = $scripturl . '?action=pm;sa=send' . (empty($_REQUEST['u']) ? '' : ';user=' . $_REQUEST['u']);
}
