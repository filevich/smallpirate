...
function loadMemberData($users, $is_name = false, $set = 'normal')
{
	...
	if ($set == 'normal')
	{
		$select_columns = "
			...
			mem.referrals_no, mem.referrals_hits, mem.referred_by, mem.referred_on,
			...
			mem.birthdate, mem.memberIP, mem.memberIP2, mem.ICQ, mem.AIM, mem.YIM, mem.MSN, mem.posts, mem.topics, mem.lastLogin,
	...
	elseif ($set == 'profile')
	{
		$select_columns = "
			...
			mem.signature, mem.personalText, mem.location, mem.gender, mem.avatar, mem.ID_MEMBER, mem.memberName,
			mem.realName, mem.emailAddress, mem.hideEmail, mem.dateRegistered, mem.websiteTitle, mem.websiteUrl,
			mem.birthdate, mem.ICQ, mem.AIM, mem.YIM, mem.MSN, mem.posts, mem.topics, mem.lastLogin, mem.karmaGood,
	...
	elseif ($set == 'minimal')
	{
		$select_columns = '
			...
			mem.thank_you_post_made, mem.thank_you_post_became,
			mem.posts, mem.topics, mem.lastLogin, mem.memberIP, mem.memberIP2, mem.lngfile, mem.ID_GROUP';
		$select_tables = '';
	}
	else
...
array('name' => '', 'href' => '', 'link' => '', 'link_text' => ''),
		'msn' => $profile['MSN'] !='' && (empty($modSettings['guest_hideContacts']) || !$user_info['is_guest']) ? array(
			'name' => &$profile['MSN'],
			'href' => 'mailto:' . $profile['MSN'],
			'link' => '<a href="mailto:' . $profile['MSN'] . '" target="_blank">' . $profile['MSN'] . '</a>',
			'link_text' => '<a href="mailto:' . $profile['MSN'] . '" target="_blank">' . $profile['MSN'] . '</a>'
		) : array('name' => '', 'href' => '', 'link' => '', 'link_text' => ''),
...
function template_include($filename, $once = false)
{
	...
	if ($file_found !== true)
	{
		...
		{
			$txt['template_parse_error'] = 'GENTE TRABAJANDO!';
			$txt['template_parse_error_message'] = 'Disculpen las molestias en minutos estara en linea la Web.<br /><br /><a href="javascript:location.reload();">ACTUALIZAR</a>.';
...
function load_amigas()
{
   global $db_prefix, $context;

   $result = db_query("
      SELECT urls_amigables
      FROM {$db_prefix}settings", __FILE__, __LINE__);
   while ($row = mysql_fetch_assoc($result))
   {
      $context[] = array(
         'amigas' => $row['urls_amigables'],
      );
   }
}
