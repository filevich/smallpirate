...
if ($_GET['do'] == 'inv')
{
	...
	$context['shop_inv']['item_count'] = mysql_result($result, 0);
	...
	$result = db_query("
		...
		WHERE inv.ownerid = {$ID_MEMBER} AND inv.itemid = it.id
...
elseif ($_GET['do'] == 'invother2')
{
	...
	foreach ($context['shop_invother'] as $key => $member)
	{
		...
		$result = db_query("
			...
			FROM ({$db_prefix}shop_inventory AS inv, {$db_prefix}shop_items AS it)
			WHERE ownerid = {$member['id']} AND inv.itemid = it.id", __FILE__, __LINE__);
