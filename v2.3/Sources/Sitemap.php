...
function ShowSiteMap() {
	global $context, $scripturl, $settings, $txt, $user_info, $db_prefix, $modSettings;
        ...
        if(loadLanguage('Sitemap') == false)
            loadLanguage('Sitemap','spanish');
	...
	$context['page_index'] = constructPageIndex($scripturl . '?action=mapadelsitio', $_REQUEST['start'], mysql_num_rows($request), 100);
	if (strpos($_SERVER['QUERY_STRING'], 'start') === false)
		$context['page_index'] = str_replace('[<b>1</b>]', '<a class="navPages" href="' . $scripturl . '?action=mapadelsitio;start=0">1</a>', $context['page_index']);
