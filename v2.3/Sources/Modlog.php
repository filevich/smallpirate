...
function ViewModlog()
{
	global $db_prefix, $txt, $modSettings, $context, $scripturl;
        ...
        if(loadLanguage('Modlog') == false)
            loadLanguage('Modlog','spanish');

	loadTemplate('Modlog');

	$context['page_title'] = $txt['mod_log'];
	...
	$context['columns'] = array(
		'action' => array('sql' => 'lm.action', 'label' => $txt['mod_unk']),
	...
	$descriptions = array(
		'lock' => $txt['mod_closed'],
		'sticky' => $txt['mod_stick'],
		'modify' => $txt['edited_post'],
		'merge' => $txt['modlog_ac_merged'],
		'split' => $txt['modlog_ac_split'],
		'move' => $txt['mod_moved'],
		'remove' => '<font style="color: #FF0000;">Eliminado</font>',
		'delete' => $txt['mod_removed'],
		'delete_member' => $txt['member_deleted'],
		'ban' => $txt['mod_baned'],
		'causa' => $txt['cause'],
		'news' => $txt['mod_news'],
		'profile' => $txt['mod_perfil'],
