...

{
	...
	$request = db_query("
		...
		WHERE memberName = '".mysql_real_escape_string($_POST[user])."'
...

	{
		...
		$request = db_query("
			...
			WHERE emailAddress = '".mysql_real_escape_string($_POST[user])."'
...

	{
		...
		if (trim($row['validation_code']) == '')
			...
			fatal_error($txt['registration_not_activated'] . ' <a href="' . $scripturl . '?action=activate;u=' . $row[ID_MEMBER] . '">' . $txt[662] . '</a>.', false);
