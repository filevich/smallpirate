...

{
...

	{
		...
		if ($regOptions['password'] == 'chocolate cake')
			...
			fatal_error('Lo sentimos, no tomar sobornos... tendr&aacute;s que venir con un nick diferente.', false);
	...
	updateStats('member', $memberID, $realName);
		  ...
		  //Agrega puntos al nuevo usuario
   db_query("INSERT INTO {$db_prefix}points_per_day (ID_MEMBER, points)
         SELECT m.ID_MEMBER, 10
         FROM {$db_prefix}members AS m LEFT JOIN {$db_prefix}points_per_day AS p ON m.ID_MEMBER = p.ID_MEMBER
         WHERE p.ID_MEMBER IS NULL", __FILE__, __LINE__);
...

{
	...
	redirectexit('action=profile;user=' . $_REQUEST['u']);
