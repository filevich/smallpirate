...

{
	...
	if (loadLanguage('PersonalMessage') == false)
		loadLanguage('PersonalMessage','spanish');
...

{
	...
	global $txt, $context, $scripturl, $sc, $modSettings, $settings, $user_info, $url;
	...
	$context['pm_areas'] = array(
		...
		'folders' => array(
			...
			'areas' => array(
				...
				'send' => array('link' => '<img border="0" alt="' . $txt[321] . '" src="'. $url .'/Themes/default/images/icons/icono-enviar-mensaje.gif" align="absmiddle"> <a href="'. $scripturl .'?action=pm;sa=send">' . $txt[321] . '</a>', 'href' => ''. $scripturl .'?action=pm;sa=send'),

				'inbox' => array('link' => '<img border="0" alt="' . $txt[316] . '" src="'. $url .'/Themes/default/images/icons/enviarmp.gif" align="absmiddle"> <a href="'. $scripturl .'?action=pm">' . $txt[316] . '</a>', 'href' => ''. $scripturl .'?action=pm'),

				'outbox' => array('link' => '<img border="0" alt="' . $txt[320] . '" src="'. $url .'/Themes/default/images/icons/mensaje_env.gif" align="absmiddle"> <a href="'. $scripturl .'?action=pm;f=outbox">' . $txt[320] . '</a>', 'href' => ''. $scripturl .'?action=pm;f=outbox'),
...

	{
...

		{
			...
			$context['pm_areas']['labels']['areas']['label' . $label['id']] = array(
				...
				'link' => '<a href="'. $scripturl .'/?action=pm;l=' . $label['id'] . '"><img border="0" alt="' . $label['name'] . '" src="'. $url .'/Themes/default/images/stats_board.gif"> ' . $label['name'] . '</a> <input class="check" name="delete_label[' . $label['id'] . ']" type="checkbox">',
...

{
	...
	$message['subject'] = $message['subject'] == '' ? 'Sin Asunto' : $message['subject'];
...

{
	...
	$context['subject'] = $form_subject != '' ? $form_subject : 'Sin Asunto';

	$context['message'] = $form_message;
...
if($to == $context['user']['name'])
	...
	fatal_error($txt['pm_auto_message']);
...

{
...

	{
...

		{
			...
			$labels = $row['labels'] == '' ? array('') : explode(',', trim($row['labels']));
...

{
...

	{
		...
		redirectExit(''. $scripturl .'?action=pm');
