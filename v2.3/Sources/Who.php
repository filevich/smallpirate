...
function Who()
{
	global $db_prefix, $context, $scripturl, $boardurl, $user_info, $txt, $modSettings, $ID_MEMBER, $memberContext;
	...
	if (empty($modSettings['who_enabled']))
        ...
        // Load the 'Who' language
        if(loadLanguage('Who') == false)
            loadLanguage('Who','spanish');
	...
	$request = db_query("
		SELECT	(UNIX_TIMESTAMP(lo.logTime) - UNIX_TIMESTAMP() + " . time() . ") AS logTime,
	...
	$result = db_query("
		SELECT	lo.ID_MEMBER, lo.logTime, mem.realName, mem.avatar, mem.money, mem.posts, mem.memberName, mem.showOnline,
			mg.onlineColor, mg.ID_GROUP, mem.avatar, mg.groupName
		FROM {$db_prefix}log_online AS lo
...
$link = '<table align="left" style="align: left; margin: 8px 1px 15px 1px;"><tr><td><div class="box_title" style="width: 290px;">
	<div class="box_txt box_7er-34"><center>' . $row['memberName'] . '</center></div>
        <div class="box_rss">
    	<div class="icon_img"><img src="'.$boardurl.'/Themes/default/images/blank.gif?v3.2.3"></div>
    </div>
</div>
<div class="windowbg" style="padding: 4px; width: 280px; -moz-border-radius-bottomright: 10px; -moz-border-radius-bottomleft: 10px;">	
<table border="0">
	<tr><td border="1" style="padding: 5px;">
		<img src="' . $row['avatar'] . '" width="120" height="165">
		</td><td style="padding: 0px 5px 5px 12px;">
		<p><font face="Arial">
		<a href="' . $scripturl . '?action=pm;sa=send;u=' . $row['ID_MEMBER'] . '">
		<span class="size11"><img src="'.$boardurl.'/Themes/default/images/im_on.gif" border="0"> '.$txt['who_send_mp'].'</span></a></span><br>
		<font face="Arial">
		<a href="' . $scripturl . '?action=profile;u=' . $row['ID_MEMBER'] . '">
		<span class="size11"><img src="'.$boardurl.'/Themes/default/images/icons/profile_sm.gif" border="0"> '.$txt['who_view_profile'].'</span></a></font>
	 	<p>      
	 	<p><strong>'.$txt['who_post'].'&#32;</strong>' . $row['posts'] . '<br>
                <strong>'.$txt['who_points'].'&#32;</strong>'. (int)$row['money'] . '<br>
		<strong>'.$txt['who_rango'].'&#32;</strong>' . $row['groupName'] . '<br>
		</font>
		<div style="margin-bottom:4px;"><span class="icons fot2"><a href="' .$scripturl. '?action=imagenes;usuario=' . $row['memberName'] . '" title="'.$txt['who_your_images'].'"> '.$txt['who_your_images'].'</a></span></div>
</td></tr></table>
</div></td></tr></table>';
