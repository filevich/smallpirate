...
function constructPageIndex($base_url, &$start, $max_value, $num_per_page, $flexible_start = false)
{
	...
	else
	{
		...
		$tmpStart = $start + $num_per_page * 1;
		if ($tmpStart <= $tmpMaxPages) $pageindex .= sprintf($base_link, $tmpStart, "Siguiente &raquo;&raquo;");
...
function adminIndex($area)
{
	...
	$context['admin_areas']['forum'] = array(
		...
		'areas' => array(
			...
			'reportlist' => '<a href="'.$scripturl.'?action=imagenes;sa=reportlist">Im&aacute;genes Reportadas</a>',
		)
	);

	if (allowedTo(array('edit_news', 'send_mail', 'admin_forum')))
		$context['admin_areas']['forum']['areas']['news'] = '<a href="' . $scripturl . '?action=news">Noticias</a>';
	...
	if (allowedTo('admin_forum'))
	{
		$context['admin_areas']['config'] = array(
			...
			'areas' => array(
				...
				'Extras' =>  '<a href="' . $scripturl . '?action=extras">Publicidad</a>',
...
//hasta aca llege... by j0n4th4ntub3
global $context, $settings, $options, $scripturl, $forum_copyright,$forum_version, $txt;
...
$temporary2 = preg_replace('~<div.+?</div>~s', '', ob_get_contents());

	

	

if (strpos($temporary2, '<div') !== false)
...
$forum_copyright = 'Powered by <a href="http://www.spirate.net/" title="Spirate" target="_blank">' . $forum_version . '</a> & 
<a href="http://www.simplemachines.org/about/copyright.php" title="Free Forum Software" target="_blank">SMF</a><br><br>';	

// For SSI and other things, detect the version.

if (!isset($forum_version) || strpos($forum_version, 'Spirate') === false || isset($_GET['checkcopyright']))
{
$data = substr(file_get_contents(__FILE__), 0, 4096);
if (preg_match('~\*\s*Software\s+Version:\s+(Spirate\s+.+?)[\s]{2}~i', $data, $match) == 0)
$match = array('', 'Spirate');
$forum_copyright = preg_replace('~(<a href="http://www.spirate.net/"[^>]+>)</a>~', '$1' . $match[1] . '</a>', $forum_copyright);
}
...
echo'<span class="smalltext" style="display: inline; visibility: visible; font-family: Verdana, Arial, sans-serif;"><center>';
if ($get_it == 'none')
{
$found = true;
//demosle una sorpresa al "dueño del script" ^^
echo '

<div style="white-space: normal;">El administrador no quiere un aviso de copyright diciendo que este es el copyright 2009 - 2010 por <a href="http://www.spirate.net/" target="_blank">Small Pirate</a>, junto con <a href="http://www.simplemachines.org/">SMF</a>, hasta que no se solucione esta situaci&oacute;n el mensaje seguir&aacute; apareciendo.</div>';
...
elseif (isset($modSettings['copyright_key']) && sha1($modSettings['copyright_key'] . 'banjo') == '1d01885ece7a9355bdeb22ed107f0ffa8c323026'){$found = true; return;}elseif ((strpos($forum_copyright, '<a href="http://www.spirate.net/" title="Spirate" target="_blank">Powered by Spirate') !== false || strpos($forum_copyright, '<a href="http://www.simplemachines.org/" onclick="this.href += \'referer.php?forum=' . urlencode($context['forum_name'] . '|' . $boardurl . '|' . $forum_version) . '\';" target="_blank">Spirate') !== false || strpos($forum_copyright, '<a href="http://www.spirate.net/" target="_blank">Spirate') !== false || strpos($forum_copyright, '<a href="http://www.spirate.net/" title="Spirate" target="_blank">Spirate') !== false)&&((strpos($forum_copyright, '<a href="http://www.simplemachines.org/about/copyright.php" title="Free Forum Software" target="_blank">SMF') !== false && (strpos($forum_copyright, 'Lewis Media</a>') !== false || strpos($forum_copyright, '</a>') !== false)) || strpos($forum_copyright, '<a href="http://www.lewismedia.com/">Lewis Media</a>') !== false || strpos($forum_copyright, '<a href="http://www.lewismedia.com/" target="_blank">Lewis Media</a>') !== false || (strpos($forum_copyright, '<a href="http://www.simplemachines.org/about/copyright.php"') !== false &&

	

strpos($forum_copyright, '') !== false))){$found = true; echo $forum_copyright;}
...
Perd&oacute;n, pero el COPYRIGHT debe estar en la plantilla.<br />
...
Por favor notifica al administrador de este sitio que el mismo no tiene el COPYRIGHT de <a href="http://www.spirate.net/" style="color: black !important; font-size: large !important;">SPIRATE</a> para que pueda rectificar la situaci&oacute;n. Tener el COPYRIGHT es un <a href="http://www.simplemachines.org/about/license.php" style="color: red;">requerimiento legal</a>. Para m&aacute;s informaci&oacute;n sobre esto visita el sitio de <a href="http://www.simplemachines.org">Simple Machines</a>.', empty($context['user']['is_admin']) ? '' : '<br />
...
No sab&eacute; por qu&eacute; est&aacute; apareciendo este mensaje?  <a href="http://www.simplemachines.org/redirect/index.php?copyright_error">Mir&aacute; estas causas m&aacute;s comunes.</a>', '
