	...
	while ($row = mysql_fetch_assoc($request))
	{
		...
		$context['favoritos'][] = array(
			...
			'poster' => array(
				'id' => $row['ID_MEMBER'],
				'name' => $row['realName'],
				'href' => empty($row['ID_MEMBER']) ? '' : '' . $scripturl . '?action=profile;user=' .$row['realName'],
				'link' => empty($row['ID_MEMBER']) ? $row['realName'] : '<a href="' . $scripturl . '?action=profile;user=' . $row['realName'] . '">' . $row['realName'] . '</a>'
			),
			'subject' => $row['subject'],
...
function addFavoritos($id_topic, $id_user = null)
{
	...
	else
	{
		$result = db_query("
						...
						(ID_MEMBER, ID_TOPIC)
						VALUES
						($id_user, $id_topic)", __FILE__, __LINE__);
