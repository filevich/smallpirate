...
mysql_free_result($request);
if($context['leecher'])
fatal_error('Los Leecher no pueden agregar imagenes.',true);
}
...
mysql_free_result($request);
if($context['leecher'])
fatal_error('Los Leecher no pueden editar imagenes.',true);
	$id = (int) $_REQUEST['id'];
	if($id == '')
		fatal_error($txt['gallery_error_no_pic_selected']);
...
redirectexit('?action=imagenes&usuario=' . $context['user']['name']);
		...
		fatal_error('Usted no tiene permisos para editar esta imagen.');
...
function AddComment2()
{
	...
	isAllowedTo('smfgallery_comment');
	$comment = strip_tags($_REQUEST['cuerpo_comment']);
	$id = (int) $_REQUEST['id'];
	if($id == '')
		fatal_error($txt['gallery_error_no_pic_selected']);
    $dbresult = db_query("SELECT p.allowcomments FROM {$db_prefix}gallery_pic as p WHERE ID_PICTURE = $id LIMIT 1", __FILE__, __LINE__);
...
function AdminSettings2()
{
	...
	$gallery_commentchoice =  isset($_REQUEST['gallery_commentchoice']);
	...
	$gallery_who_viewing = isset($_REQUEST['gallery_who_viewing']);
	...
	updateSettings(
	array('gallery_max_height' => $gallery_max_height,
...
function CatUp()
{
	...
	if(db_affected_rows()== 0)
		fatal_error('There is no category above the current one.',false);
