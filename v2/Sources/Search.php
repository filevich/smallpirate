...
$request = db_query("
	...
	if (isset($_REQUEST['params']))
	{
		...
		if (isset($context['search_params']['categoria']))
			$context['search_params']['categoria'] = $context['search_params']['categoria'] == '' ? array() : explode(',', $context['search_params']['categoria']);
	}
	if (isset($_REQUEST['q']))
		$context['search_params']['q'] = stripslashes(un_htmlspecialchars($_REQUEST['q']));

	if (isset($context['search_params']['q']))
		$context['search_params']['q'] = htmlspecialchars($context['search_params']['q']);
	if (isset($context['search_params']['autor']))
		$context['search_params']['autor'] = htmlspecialchars(stripslashes($context['search_params']['autor']));
	...
	while ($row = mysql_fetch_assoc($request))
	{
		...
		$context['categories'][$row['ID_CAT']]['boards'][$row['ID_BOARD']] = array(
			...
			'name' => $row['name'],
			'child_level' => $row['childLevel'],
			'selected' => (empty($context['search_params']['categoria']) && (empty($modSettings['recycle_enable']) || $row['ID_BOARD'] != $modSettings['recycle_board'])) || (!empty($context['search_params']['categoria']) && in_array($row['ID_BOARD'], $context['search_params']['categoria']))
		);
	}
	mysql_free_result($request);
...
$request = db_query("
	...
	if (isset($_REQUEST['params']))
	{
		...
		if (isset($search_params['categoria']))
			$search_params['categoria'] = empty($search_params['categoria']) ? array() : explode(',', $search_params['categoria']);
	}
	...
	if (!empty($search_params['autor']) || (!empty($_REQUEST['autor']) && $_REQUEST['autor'] != '*'))
		$search_params['autor'] = isset($search_params['autor']) ? $search_params['autor'] : $_REQUEST['autor'];

	// If there's no specific user, then don't mention it in the main query.
	if (empty($search_params['autor']))
		$userQuery = '';
	else
	{
		$userString = strtr(addslashes($func['htmlspecialchars'](stripslashes($search_params['autor']), ENT_QUOTES)), array('&quot;' => '"'));
	...
	if (!empty($search_params['categoria']) && is_array($search_params['categoria']))
		$_REQUEST['categoria'] = $search_params['categoria'];
	if (!is_array($_REQUEST['categoria']) && !empty($_REQUEST['categoria']))
		$_REQUEST['categoria'] = strpos($_REQUEST['categoria'], ',') !== false ? explode(',', $_REQUEST['categoria']) : array($_REQUEST['categoria']);
	if (!empty($_REQUEST['categoria']))
		foreach ($_REQUEST['categoria'] as $id => $categoria)
			$_REQUEST['categoria'][$id] = (int) $categoria;
	if (!empty($search_params['topic']))
	{
		...
		$search_params['categoria'] = array();
		list ($search_params['categoria'][0]) = mysql_fetch_row($request);
		mysql_free_result($request);
	}
	// Select all boards you've selected AND are allowed to see.
	elseif ($user_info['is_admin'] && (!empty($search_params['advanced']) || !empty($_REQUEST['categoria'])))
		$search_params['categoria'] = empty($_REQUEST['categoria']) ? array() : $_REQUEST['categoria'];
	else
	{
		$request = db_query("
			SELECT b.ID_BOARD
			FROM {$db_prefix}boards AS b
			WHERE $user_info[query_see_board]" . (empty($_REQUEST['categoria']) ? (!empty($modSettings['recycle_enable']) && $modSettings['recycle_board'] > 0 ? "
				AND b.ID_BOARD != $modSettings[recycle_board]" : '') : "
				AND b.ID_BOARD IN (" . implode(', ', $_REQUEST['categoria']) . ")"), __FILE__, __LINE__);
		$search_params['categoria'] = array();
		while ($row = mysql_fetch_assoc($request))
			$search_params['categoria'][] = $row['ID_BOARD'];
		mysql_free_result($request);
		if (empty($search_params['categoria']))
			$context['search_errors']['no_boards_selected'] = true;
	}
	if (count($search_params['categoria']) != 0)
	{
		$request = db_query("
			SELECT COUNT(*)
		...
		if (count($search_params['categoria']) == $num_boards)
			$boardQuery = '';
		elseif (count($search_params['categoria']) == $num_boards - 1 && !empty($modSettings['recycle_board']) && !in_array($modSettings['recycle_board'], $search_params['categoria']))
			$boardQuery = '!= ' . $modSettings['recycle_board'];
		else
			$boardQuery = 'IN (' . implode(', ', $search_params['categoria']) . ')';
	}
	else
		$boardQuery = '';
	...
	if (empty($search_params['sort']) && !empty($_REQUEST['sort']))
		list ($search_params['sort'], $search_params['sort_dir']) = array_pad(explode('q', $_REQUEST['sort']), 2, '');
	...
	if ($context['show_spellchecking'])
	{
		...
		if ($found_misspelling)
		{
			...
			if (isset($temp_params['categoria']))
				$temp_params['categoria'] = implode(',', $temp_params['categoria']);
	...
	if (isset($context['search_params']['q']))
		$context['search_params']['q'] = $func['htmlspecialchars']($context['search_params']['q']);
	if (isset($context['search_params']['autor']))
		$context['search_params']['autor'] = $func['htmlspecialchars']($context['search_params']['autor']);
	...
	if (!empty($context['topics']))
	{
		...
		$messages_request = db_query("
			SELECT
				m.ID_MSG, m.subject, m.posterName, m.posterEmail, m.posterTime, t.puntos, m.ID_MEMBER,
...
$request = db_query("
	...
	$output = array_merge($context['topics'][$message['ID_MSG']], array(
		'first_post' => array(
			...
			'href' => $scripturl . '?topic=' . $message['ID_TOPIC'] . '',
			'link' => '<a href="'. $scripturl .'?topic=' . $message['ID_TOPIC'] . '">' . $message['first_subject'] . '</a>',
			'member' => array(
				'id' => $message['first_member_id'],
				'name' => $message['first_member_name'],
				'nombre' => $message['memberName'],
				'href' => !empty($message['first_member_id']) ? ''. $scripturl .'?action=profile;user=' . $message['memberName'] : '',
				'link' => !empty($message['first_member_id']) ? '<a href="'. $scripturl .'?action=profile;user=' . $message['memberName'] . '" title="' . $txt[92] . ' ' . $message['first_member_name'] . '">' . $message['first_member_name'] . '</a>' : $message['first_member_name']
			)
		),
		...
		'board' => array(
			...
			'link' => '<a href="'. $scripturl .'?id=' . $message['ID_BOARD'] . '">' . $message['bName'] . '</a>'
		),
	));
