...
function actualizar_comentarios() {
    $.ajax({
        type: "POST",
        url: "http://www.morphcreations.es/yelid/comentarios.php",
...
function citar_comment(id){
  var obj = document.forms.postmodify.comentario;
  var user = el('autor_cmnt_' + id).getAttribute('user_comment');
  var text = el('autor_cmnt_' + id).getAttribute('text_comment');
  obj.value=obj.value + '[quote=' + user + ']' + text + '[/quote]\n';
  obj.focus();
}
