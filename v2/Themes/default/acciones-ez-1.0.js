...
function actualizar_comentarios(){
  $('#last_comments').slideUp(1);
  $.ajax({
    type: 'POST',
    url: '/comentarios.php',
    success: function(h){
      $('#last_comments').html(h);
      $('#last_comments').slideDown({duration: 1000, easing: 'easeOutBounce'});
    }
  });
}
/*Crear carpetas*/
...
function karma(id, action, path, t_prefix){
    xhr.open('get', 'http://casitaweb.net/comentario/puntuar-'+ id +'-'+ action +'-'+ path +'-'+ t_prefix);
...
function handleResponse(){
    ...
    var response = xhr.responseText.split('|');
        ...
        if(response[0] == 'done'){
            if(response[1]){
                document.getElementById("down-"+response[1]).src        = 'http://casitaweb.net/Themes/default/images/icons/gris_down.png';
                document.getElementById("down-"+response[1]).onclick    = '';
                document.getElementById("up-"+response[1]).src          = 'http://casitaweb.net/Themes/default/images/icons/gris_up.png';
